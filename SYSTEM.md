...
- **Compatibilidad Maxima con Computo Serverless**: Todo el codigo del proyecto debe ser 100% compatible con Databricks Serverless Compute. Esta PROHIBIDO el uso de `spark.sparkContext` y cualquier acceso directo al JVM del driver (error `JVM_ATTRIBUTE_NOT_SUPPORTED`). Esto incluye pero no se limita a: `spark.sparkContext.broadcast()`, `spark.sparkContext.parallelize()`, `spark.sparkContext.accumulator()`, `spark.sparkContext.setJobGroup()`, y cualquier otro metodo de `sparkContext`. En su lugar, se deben usar alternativas nativas de PySpark o variables Python capturadas por closure (cloudpickle). Esta regla aplica a todos los notebooks del proyecto sin excepcion.
- **Protocolo Obligatorio abfss:// para Lectura y Escritura de Parquets**: Toda ruta de lectura o escritura de archivos parquet debe usar exclusivamente el protocolo `abfss://` (Azure Blob File System Secure) para acceder a Azure Data Lake Storage Gen2. El formato obligatorio es: `abfss://container@storageaccount.dfs.core.windows.net/ruta/del/recurso`. Esta PROHIBIDO el uso de rutas `/mnt/` (DBFS mounts) ya que son un mecanismo legacy que no es compatible con las mejores practicas de Unity Catalog y External Locations.
...
- **Version 6**: Generar los siguientes documentos en formato markdown (.md) en la carpeta docs:
  - ManualTecnico.md: Es un Manual que explica a detalle y de forma técnica los Notebooks de Generación de Parquets y todo el Lakeflow Spark Declarative Pipelines. La explicación es técnica y al detalle, por ejemplo: 1. Explicar que son los decoradores @dp. 2-explicar como es que funciona el paragdigma declarative que usa Lakeflow Spark Declarative Pipelines. 3- Explicar las propiedades de las tablas deltas streaming y de las vistas materializadas. 4- toda las operaciones lígicas hechas con el API de Data Frames de spark. 5- Entre otro. Adicionalmente en este manual técnico debe venir un apartado dedicado a todos los parámetros que necesitan los notebooks y el Lakeflow Spark Declarative Pipelines, así como, la explicación de la tabla dbo.Parametros perteneciente al Azure SQL con el detalle de todos sus claves y valores. Por ultimo, entre las primeras seciones de este manual debe venir una sección de Dependencias en el cual se debe explicar que el usuario ya debe tener los siguientes recursos instalados y configruados en Azure:
    - Azure Storage Account de tipo Data Lake Storage Gen2, con los contenedores bronce, plata, oro y control creados
    - Azure SQL con el Free Tier activo
    - El Azure SQL con la base de datos llamada Configuracion y dentro de esta la tabla dbo.Parametros con todos los registros cargados
    - El Azure Key Vault aprovisionado y los dos secretos creados
    - Azure Databricks con Tier Premiun configurado correctamente y con el Unity Catalog habilitado.
    - Azure Databricks con los external location referenciando a los contenedores del Data Lake Storage Gen2
    - Azure Databricks con los Catalogos creados: bronce_dev, plata_dev, oro_dev y control_dev, además, el esquema regional creado en cada catalogo.
  - ModeladoDatos.md: Este documento muestra el modelado de los datos, desde la composición de los campos/columnas de los archivos parquets, sus tipos de datos y descripciones, hasta las vistas materializadas y las streaming tables de bronce, igualmente con campos/columnas, tipos de datos y descripciones. En el caso de los campos calculados, le descripción del mismo debe explicar la lógica aplicada y los campos que interactuan en esta lógica. Por ultimo, cada uno de los modelos descritos debe indicar su nombre: el nombre de los parquets en el caso de los datos de los parquets y el nombre de las streaming tables y las vistas materializadas para el detalle de cada una de ellas.
