...
# MAGIC %md
# MAGIC # LsdpBronceCmstfl — Tabla Streaming de Bronce: Maestro de Clientes
# MAGIC
# MAGIC **Proposito**: Ingestar los parquets del Maestro de Clientes (CMSTFL — Customer Master File)
# MAGIC desde ADLS Gen2 hacia la tabla streaming `bronce_dev.regional.cmstfl` usando AutoLoader
...
# MAGIC - Tabla: `bronce_dev.regional.cmstfl`
# MAGIC - Columnas: 72 (70 AS400 + FechaIngestaDatos + _rescued_data)
...
# MAGIC construir las 2 rutas `abfss://` necesarias para la tabla `cmstfl`:
# MAGIC - `ruta_parquets_maestro`: directorio de parquets del Maestro de Clientes
# MAGIC - `ruta_checkpoint_cmstfl`: directorio de checkpoint (schemaLocation de AutoLoader)
# MAGIC
...
# Parametro del pipeline: rutaCompletaMaestroCliente (ej: "LSDP_Base/As400/MaestroCliente/")
ruta_relativa_parquets = spark.conf.get("pipelines.parameters.rutaCompletaMaestroCliente")

# Parametro del pipeline: rutaCheckpointCmstfl (ej: "LSDP_Base/Checkpoints/Bronce/cmstfl/")
ruta_relativa_checkpoint = spark.conf.get("pipelines.parameters.rutaCheckpointCmstfl")

# Construir la ruta abfss:// completa al directorio de parquets del Maestro de Clientes.
# Ejemplo resultado: abfss://bronce@adlsg2datalakedev.dfs.core.windows.net/archivos/LSDP_Base/As400/MaestroCliente/
ruta_parquets_maestro = construir_ruta_abfss(
...
# Construir la ruta abfss:// completa para el checkpoint (schemaLocation) de cmstfl.
# El checkpoint almacena la informacion de esquema inferida por AutoLoader (R7-D1).
# Ejemplo resultado: abfss://bronce@adlsg2datalakedev.dfs.core.windows.net/archivos/LSDP_Base/Checkpoints/Bronce/cmstfl/
ruta_checkpoint_cmstfl = construir_ruta_abfss(
    parametros_sql["contenedorBronce"],
    parametros_sql["datalake"],
    parametros_sql["DirectorioBronce"],
...
# MAGIC %md
# MAGIC ## Definicion de la Tabla Streaming `cmstfl`
# MAGIC
# MAGIC Tabla `@dp.table` que ingesta los parquets del Maestro de Clientes via AutoLoader.
# MAGIC
...
# MAGIC
# MAGIC **Variables del modulo capturadas por closure**:
# MAGIC `ruta_parquets_maestro` y `ruta_checkpoint_cmstfl` (compatible con Serverless — RF-011).
...
@dp.table(
    name="cmstfl",
    comment=(
        "Tabla streaming de bronce — Maestro de Clientes (Customer Master File). "
...
def tabla_bronce_cmstfl():
    """
    Lee los parquets del Maestro de Clientes en modo streaming via AutoLoader
    y agrega la columna FechaIngestaDatos con la marca de tiempo de la ingesta.

    Las variables ruta_parquets_maestro y ruta_checkpoint_cmstfl son del modulo
    y quedan capturadas por closure (cloudpickle) — sin spark.sparkContext (RF-011).

    Resultado: 72 columnas (70 AS400 + FechaIngestaDatos + _rescued_data automatica).
    """
    df_stream = (
        spark.readStream
        .format("cloudFiles")
