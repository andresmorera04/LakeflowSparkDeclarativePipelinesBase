...
# MAGIC %md
# MAGIC # NbTestBronceBlncfl — Suite de Pruebas TDD: Tabla Streaming blncfl
# MAGIC
# MAGIC **Proposito**: Validar la correcta creacion e ingesta de la tabla streaming
# MAGIC `bronce_dev.regional.blncfl` (Saldos de Clientes) por el pipeline LSDP de bronce.
# MAGIC
# MAGIC **Prerequisito**: Ejecutar el pipeline LSDP de bronce al menos una vez antes de
# MAGIC correr estas pruebas. La tabla `bronce_dev.regional.blncfl` debe existir en Unity Catalog.
# MAGIC
# MAGIC **Plataforma**: Computo Serverless de Databricks.
# MAGIC
# MAGIC **Cobertura**:
# MAGIC 1. Tabla bronce_dev.regional.blncfl existe en Unity Catalog
...
# MAGIC %md
# MAGIC ## Prueba 1 — La tabla bronce_dev.regional.blncfl existe en Unity Catalog
# MAGIC
...
try:
    df_blncfl = spark.table("bronce_dev.regional.blncfl")
    print("PRUEBA 1 PASA: La tabla bronce_dev.regional.blncfl existe en Unity Catalog.")
except Exception as error:
    raise AssertionError(
        "ERROR — Prueba 1: La tabla bronce_dev.regional.blncfl NO existe en Unity Catalog. "
        f"Error: {error}. "
...
columnas_actuales = df_blncfl.columns

assert "FechaIngestaDatos" in columnas_actuales, (
    "ERROR — Prueba 2: La columna 'FechaIngestaDatos' no existe en la tabla blncfl. "
    "Verificar que el pipeline incluye '.withColumn(\"FechaIngestaDatos\", current_timestamp())'."
)

tipo_fecha_ingesta = dict(df_blncfl.dtypes)["FechaIngestaDatos"]
...
assert "_rescued_data" in columnas_actuales, (
    "ERROR — Prueba 3: La columna '_rescued_data' no existe en la tabla blncfl. "
    "Verificar que el pipeline no tiene un schema explicito en @dp.table (R7-D2)."
)

tipo_rescued = dict(df_blncfl.dtypes)["_rescued_data"]
...
df_detalle = spark.sql("DESCRIBE DETAIL bronce_dev.regional.blncfl")
...
# MAGIC FechaIngestaDatos (primer campo) y CUSTID (FK al Maestro, clave de dimension tipo 1 en plata).

# COMMAND ----------

df_detalle_cluster = spark.sql("DESCRIBE DETAIL bronce_dev.regional.blncfl")
...
# MAGIC
# MAGIC Verifica que AutoLoader proceso los parquets de Saldos correctamente.

# COMMAND ----------

conteo_registros = df_blncfl.count()

assert conteo_registros > 0, (
    "ERROR — Prueba 6: La tabla blncfl esta vacia (0 registros). "
    "Verificar que: (1) los parquets de saldos existen en la ruta abfss:// configurada, "
    "(2) el pipeline se ejecuto correctamente."
)

print(f"PRUEBA 6 PASA: La tabla blncfl tiene {conteo_registros:,} registros ingestados.")
...
print("=" * 60)
print("RESUMEN — NbTestBronceBlncfl")
print("=" * 60)
print("Prueba 1: Tabla blncfl existe en Unity Catalog      [PASA]")
