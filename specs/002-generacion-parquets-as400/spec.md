...
- **RF-014**: La Version 2 DEBE contar con una suite de pruebas TDD ubicada en `tests/GenerarParquets/` que valide: estructura de columnas, volumetria, integridad referencial entre tablas, tipos de datos, exclusion de nombres latinos, y codigos de tipos de transaccion contra el catalogo aprobado. Las restricciones de compatibilidad Serverless (prohibicion de `spark.sparkContext` y `broadcast()`) se verifican mediante revision de codigo y no mediante pruebas TDD automatizadas, ya que su uso produce un error directo en tiempo de ejecucion (`JVM_ATTRIBUTE_NOT_SUPPORTED`).
